...
TEST(UtilsCryptoTest, Sha256EmptyInput)
{
    ...
    auto hash = functions::crypto::sha256(empty);
    EXPECT_EQ(hash.size(), types::vars::SHA256_KEY_SIZE);
...
TEST(UtilsCryptoTest, Sha256KnownInput)
{
    ...
    auto hash = functions::crypto::sha256(data);
...
TEST(UtilsCryptoTest, HmacSha256BasicProperties)
{
    ...
    auto mac1 = functions::crypto::hmacSha256(key, data);
    auto mac2 = functions::crypto::hmacSha256(key, data);
    EXPECT_EQ(mac1.size(), types::vars::SHA256_KEY_SIZE);
    EXPECT_EQ(mac1, mac2);

    // Changing key alters result
    std::vector<uint8_t> key2 = {0x04, 0x05, 0x06};
    auto macKey2 = functions::crypto::hmacSha256(key2, data);
    EXPECT_NE(mac1, macKey2);

    // Changing data alters result
    std::vector<uint8_t> data2 = {0x0d};
    auto macData2 = functions::crypto::hmacSha256(key, data2);
...
TEST(UtilsCryptoTest, D2iPubkeyFromVectorInvalidInput)
{
    ...
    EVP_PKEY* evpPkey1 = functions::crypto::d2iPubKeyFromVector(empty);
    EXPECT_EQ(evpPkey1, nullptr);

    std::vector<uint8_t> invalid = {0x00, 0x01, 0x02};
    EVP_PKEY* evpPkey2 = functions::crypto::d2iPubKeyFromVector(invalid);
    EXPECT_EQ(evpPkey2, nullptr);
}
